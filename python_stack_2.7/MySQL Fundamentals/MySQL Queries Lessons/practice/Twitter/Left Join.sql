...
-- What query would you run to get all the users that are following the user with id 1?(self-join)
...
-- What query would you run to get all users that are not following the user with id of 2?(nested queries)
SELECT  * 
FROM users
...
-- Left Join vs. Join
SELECT first_name, tweet -- outer join
FROM users
LEFT JOIN tweets
ON users.id = tweets.user_id;

SELECT first_name, tweet -- inner join
FROM users  
JOIN tweets
ON users.id = tweets.user_id
